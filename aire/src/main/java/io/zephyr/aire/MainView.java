...
public class MainView extends VerticalLayout {
  ...
  public MainView(GreetService service, final Kernel kernel) {
    ...
    TextField textField = new TextField(kernel.toString());



    // Button click listeners can be defined as lambda expressions
    Button button =
        new Button("Say hello", e -> Notification.show(service.greet(textField.getValue())));
    ...
    add(new Button("Done"));
    for(Module module : kernel.getModuleManager().getModules()) {
      add(new Button(module.getCoordinate().toCanonicalForm()));
    }
  }
}
