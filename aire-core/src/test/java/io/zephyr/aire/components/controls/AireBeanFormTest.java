...
class AireBeanFormTest {
  ...
  //    @ViewTest(MainView.class)
  //    @EditView(withMethods = "basicSetup")
  //    void ensureFormDoesNotCreateFieldForNonFormFieldProperties(@Elements("//label") List<Label>
  // labels) {
  //        val label = labels.stream().filter(t ->
  // t.getText().contains("dontDisplayMe")).findAny();
  ...
  void ensureFormCreatesFieldForFormFieldProperties() {
    ...
    val displayable =
        form.getChildren()
            ...
            .filter(field -> field.getLabel().equals("displayMe"));
  ...
  void ensureFormDoesNotCreateFieldForNonFormFieldProperties() {
    ...
    val displayable =
        form.getChildren()
            .filter(field -> field instanceof TextField)
            .map(field -> (TextField) field)
            .filter(field -> field.getLabel().equals("dontDisplayMe"));
    assertEquals(0, displayable.count());
  }

  @ViewTest(MainView.class)
  @EditView(withMethods = "basicSetup")
  void ensureSubClassesGetRendered(
      @Elements("//*[@class='aire-bean-form']") List<AireBeanForm> forms) {
    assertEquals(2, forms.size());
  }

  @ViewTest(MainView.class)
  @EditView(withMethods = "basicSetup")
  void ensureOuterFormGetsButtons(
      @Element("//*[@class='aire-bean-form']/div[@class='aire-button-group']") Div divs) {
    val buttons =
        divs.getChildren()
            .filter(button -> button instanceof Button)
            .map(button -> ((Button) button).getText())
  ...
  @ViewTest(MainView.class)
  @EditView(withMethods = "basicSetup")
  void ensureSubClassFieldsGetRendered(
      @Element("//*[@class='aire-bean-form']/*[@class='aire-bean-form']") AireBeanForm form) {
    val displayable =
        form.getChildren()
  ...
  void ensureFieldsCanHaveALabelOtherThanTheirFieldName() {
    ...
    val labels =
        form.getChildren()
            .filter(field -> field instanceof TextField)
            .map(field -> ((TextField) field).getLabel())
            .collect(Collectors.toList());
  ...
  @ViewTest(MainView.class)
  @EditView(withMethods = "basicSetup")
  void ensureBigDecimalFieldsCanBeAdded(
      @Element("//vaadin-big-decimal-field") BigDecimalField field) {
  ...
  void ensureNumberFieldsCanBeAdded(@Elements("//vaadin-number-field") List<NumberField> fields) {
    assertEquals(
        1, fields.stream().filter(field -> field.getLabel().equals("thisIsANumber")).count());
    assertEquals(
        1, fields.stream().filter(field -> field.getLabel().equals("thisIsAnInt")).count());
  }
  ...
  void ensureEnumCanBeAddedAsSelect() {
    val form = new AireBeanForm<>(TestFormClass.class);
    val fields =
        form.getChildren().filter(field -> field instanceof Select).map(field -> (Select) field);
    assertEquals(1, fields.count());
  }
  ...
  static class TestFormClass {
    @AireBeanForm.FormField(
        ...
        validation =
            @AireBeanForm.FieldValidation(type = AireBeanForm.Validations.REGEXP, value = "*"))
    ...
    @AireBeanForm.FormField private Date anniversary;

    @AireBeanForm.FormField private boolean coolbean;
    ...
    @AireBeanForm.FormField private String displayMe;

    @AireBeanForm.FormField private AireSize size;

    @AireBeanForm.FormField(options = AireBeanForm.Options.Select.class)
    private AireSize otherSize;

    @AireBeanForm.FormField private BigDecimal theBiggestDecimal;

    @AireBeanForm.FormField private Number thisIsANumber;

    @AireBeanForm.FormField private Integer thisIsAnInt;

    static class SubTest {

      @AireBeanForm.FormField private int myNumber;

      @AireBeanForm.FormField private String subField;
    }
  }
}
